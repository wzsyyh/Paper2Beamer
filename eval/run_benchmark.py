...
def parse_narrative_arc_output(output: str) -> Optional[Dict[str, float]]:
    """从narrative_arc/run_evaluation.py的输出中解析叙事结构分数。"""
    try:
        data = json.loads(output)
        if "score" in data:
            scores = {"narrative_arc_score": float(data["score"])}
            logger.info(f"找到叙事结构分数: {scores}")
            return scores
    except json.JSONDecodeError as e:
        logger.error(f"解析叙事结构输出失败 (JSON): {e}")
    except Exception as e:
        logger.error(f"解析叙事结构输出失败: {e}")
    logger.warning("无法从叙事结构输出中解析分数。")
    return None
...
def main():
    ...
    for paper_dir in paper_dirs:
        ...
        logger.info(f"步骤 2c: 估叙事结构完整性 {tex_path}")
        narrative_command = ["python3", "eval/narrative_arc/run_evaluation.py", "--tex_file", str(tex_path)]
        success, narrative_stdout, _ = run_command(narrative_command)
        if success:
            narrative_scores = parse_narrative_arc_output(narrative_stdout)
            if narrative_scores:
                current_paper_scores.update(narrative_scores)
    ...
    print("\n--- 指标 2.1: 叙事结构完整性 ---")
    if "narrative_arc_score" in avg_scores:
        print(f"平均叙事结构分数: {avg_scores['narrative_arc_score']:.4f}")
    print("="*40)
